...
1. `HAMTDirectory` threshold: max `Directory` size before switching to `HAMTDirectory`. Size can be estimated by link count (naive), `PBNode.Links` size (name + CID), or full dag-pb block size (most accurate).
