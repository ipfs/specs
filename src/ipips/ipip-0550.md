...
the historical DAG-PB codec specification: blocks produced with the opt-in
`data-first` setting are valid UnixFS.
...
- **Change the canonical encode order in the DAG-PB codec specification**
  ...
  content. Ordering must remain an opt-in choice at the UnixFS layer.
