...
- `AdvisoryTTL`: TTL in nanoseconds indicating how long the record will be cached. The reference server returns its own configured value and ignores the one from the request, but that is server-specific: the Bitsocial tracker echoes the requested value back unchanged while enforcing a fixed 24-hour retention that never appears in the response. The field is advisory in name only, and no known client reads it (see Refresh cadence below), so it cannot be relied on for scheduling republication.
...
- **Address policy**: Nothing defines what a server may do with `Addrs`. The reference server passes them to the backing service verbatim, index-provider forwards them to IPNI unchanged (or ignores them wholesale when `ProviderID` and `Addrs` are configured), and Bitsocial trackers drop announced addresses that do not match the request IP and rewrite Kubo's unspecified `0.0.0.0` to it. Filtering, rewriting, and reachability checks are all server-defined. Rewriting is in tension with signature verification: a server that substitutes or filters `Addrs` stores a record that no longer matches the signature it received, which matters as soon as anything re-serves or forwards the signed record instead of only indexing it.
- **Refresh cadence**: Nothing consumes the response `AdvisoryTTL`: the reference client discards the returned value and re-announces on its own schedule (Kubo defaults to `Provide.DHT.Interval`, 22 hours, which stays under index-provider's default 24-hour expiry only by the choice of defaults). On the server side, index-provider treats a repeated announcement as a refresh: it resets the record's clock to the server's own time and publishes no duplicate IPNI advertisement. Expiry is also not the only reason to re-announce: addresses rotate (WebRTC certhashes, re-issued AutoTLS names) while the announced keys stay the same, and the API has no way to express "same keys, new addrs". A client whose browser-dialable addresses churn has to re-announce the affected keys in full, far more often than any TTL suggests. Bitsocial apps poll their own node once a minute for such changes and re-announce the routing CIDs a browser needs to bootstrap into a community.
...
- **Bitsocial trackers**: [Bitsocial](https://bitsocial.net) (previously named [Plebbit](https://plebbit.com)) is a peer-to-peer social media protocol whose apps, such as [Seedit](https://seedit.app), bundle a Kubo node. These nodes run with the public DHT disabled and announce content over this API to volunteer-run trackers whose default endpoints are hardcoded in the [pkc-js client SDK](https://github.com/pkcprotocol/pkc-js) (PKC, "Public Key Communities", is the project's name for its protocol layer). Turning the DHT off wholesale is a workaround rather than the goal; what the deployment wants is to keep a handful of keys in the DHT and everything else on HTTP routers, which needs [ipfs/kubo#11411](https://github.com/ipfs/kubo/issues/11411). The [pkc-http-router tracker server](https://github.com/pkcprotocol/pkc-http-router) stores only `Keys`, `ID`, and `Addrs`, does not verify signatures, and instead requires announced addresses to match the IP the request came from. pkc-js never builds or signs a record itself: it calls Kubo's `routing.provide` RPC and Kubo signs and PUTs, so records on this path carry valid signatures. That was not true until August 2026, when a local proxy on the client machine still [rewrote `Payload.Addrs`](https://github.com/pkcprotocol/pkc-js/blob/49f6b9c5028a870dcaaae08a5a709e9b5b506600/src/runtime/node/addresses-rewriter-proxy-server.ts) after Kubo had signed the record, without re-signing it. The proxy was [removed](https://github.com/pkcprotocol/pkc-js/pull/263) once Kubo 0.43.0 stopped dropping browser-dialable transports from records PUT to HTTP routers ([ipfs/kubo#11394](https://github.com/ipfs/kubo/pull/11394)).
...
At the time of writing this IPIP (2025Q4), there is no vendor-agnostic alternative to this HTTP API. A future IPIP could address this gap by extending :cite[http-routing-v1] with a modern specification for delegated publishing of provider information over HTTP. Such work should incorporate lessons from this historic API and support both Bitswap and HTTP ([Trustless Gateway](https://specs.ipfs.tech/http-gateways/trustless-gateway/)) providers natively. It should also let a server state the cadence it enforces, the way a BitTorrent tracker does: this API carries a server-to-client TTL that no client reads, and offers no way to advertise an announce interval or a minimum interval, so a server can neither evict on a published schedule nor reject clients that announce too often.
