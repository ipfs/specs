...
> This document is kept as the record of a threat model discussion, and is not
> proposed for adoption in its present form. The verification it describes
> guards `/tls/http` while the same traffic can be aimed at the same port by
> announcing `/tls/ws`, it places an outbound HTTP request inside the
> `ADD_PROVIDER` path of every DHT Server, and it protects nobody until nearly
> every Server enforces it. The sections "Nothing is unique to `/tls/http`",
> "Compatibility" and "Security" carry the reasoning.
...
### Nothing is unique to `/tls/http`

[kubo#11333](https://github.com/ipfs/kubo/pull/11333) serves libp2p and HTTP on
a single TCP port: `/tls/ws` and `/tls/http` share one listener, one
certificate, and one peer identity. Verifying the second while ignoring the
first draws a line the wire does not.

The asymmetry this document rests on, that a bogus libp2p address fails fast
during the security handshake, protects the dialing client rather than the
announced host. A peer that advertises `/ip4/A.B.C.D/tcp/443/tls/ws` for a
victim's HTTPS server makes every dialer complete a TCP connection and a full
TLS handshake against it, and the victim performs the asymmetric operation
before the libp2p peer ID check rejects the connection. For the target, that
costs more than answering the cached GET a `/tls/http` announcement would have
produced.

An attacker denied `/tls/http` therefore relabels the same address as `/tls/ws`
or plain `/tcp`, and the fan-out is unchanged. Verification scoped to one
protocol changes which string gets abused, not whether the DHT can be aimed at
a third party.

If address verification is worth having, it has to cover every announced
multiaddr, which is a far larger proposal than this one: proving that a peer
controls a host, whether by dial-back or by an identity-bound certificate. The
second already exists in practice, since an AutoTLS address is bound to the
peer's own key and needs no well-known file, while an address the peer does not
control stays unverifiable whichever protocol it names.
...
multiaddresses with provider records.

The check protects clients only once most DHT Servers perform it. A provider
record is stored on the 20 DHT Servers closest to the Content Kademlia
Identifier, so a client asking any Server that has not upgraded still receives
the unverified address. Until adoption is near universal, the guarantee is
partial while the cost is immediate.

Providers advertising content with `/tls/http` multiaddresses to the Amino DHT
MUST comply with the described check. Such addresses are no longer hypothetical:
[kubo#11333](https://github.com/ipfs/kubo/pull/11333) adds
`HTTPProvider.AnnounceMultiaddrs`, which derives a `/tls/http` multiaddr from
every `/tls/ws` listener on the node and announces it through identify, the
Amino DHT and IPNI. That endpoint shares the TCP port and TLS certificate of the
WebSocket listener, so if it ships, the typical `/tls/http` provider is an
ordinary node rather than a static hosting service.
...
#### Amplification through the verification itself

The check is an outbound HTTP GET from every DHT Server that receives the
record, so a single `ADD_PROVIDER` can produce up to one request per Server. The
negative cache limits repetition for a given Server and address, but an attacker
who varies the announced host, port or subdomain earns a fresh fan-out each
time, aimed wherever they choose. Implementations therefore need a rate limit on
outbound verification that does not depend on the cache.

#### Verification targets

The address being checked is attacker-controlled input, and the request is made
by the DHT Server itself. Implementations MUST NOT verify addresses pointing at
loopback, link-local, or private ranges, otherwise a public DHT Server becomes a
probe against its own network.
...
#### Offline delegation signed by the HTTP operator

Rather than every DHT Server fetching a file, the HTTP operator signs a
statement naming the authorized peer ID and an expiry, and the provider carries
it in the record. Verification becomes local and offline, the HTTP server
answers no requests at all, and the same artifact serves IPNI and the Delegated
Routing HTTP API. The cost is a new signed object, plus a key management story
for HTTP operators who have no libp2p identity today.

#### Verification by the retrieving client

The client is about to contact the endpoint anyway, so it can perform the
well-known check itself on first use. This adds no traffic the target was not
already going to receive, and asks nothing of DHT Servers. It protects only
clients that implement it, and offers no deduplication across clients.
